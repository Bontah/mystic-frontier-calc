 ...
 */
function calculateKeywordScore(text1, text2) {
 ...
 */
export function extractBonusValues(text) {
    const result = {
        flat: null,
        mult: null,
    };
 ...
 */
function calculateBonusValueMatch(extracted, bonus) {
    let score = 0;
    let checks = 0;
    if (extracted.flat !== null && bonus.flatBonus !== undefined) {
        checks++;
        if (extracted.flat === bonus.flatBonus)
            score += 100;
        else if (Math.abs(extracted.flat - bonus.flatBonus) <= 1)
            score += 50;
    }
    if (extracted.mult !== null && bonus.multiplierBonus !== undefined) {
        checks++;
        if (Math.abs(extracted.mult - bonus.multiplierBonus) < 0.05)
            score += 100;
        else if (Math.abs(extracted.mult - bonus.multiplierBonus) < 0.2)
            score += 50;
    }
 ...
 */
export function matchConditionalText(extractedText) {
    const state = store.getState();
    const bonuses = state.configConditionalBonuses.bonuses;
    if (!extractedText || !bonuses)
        return null;
    ...
    for (const bonus of bonuses) {
        ...
        const finalScore = bonusValueScore > 0 ? textScore * 0.7 + bonusValueScore * 0.3 : textScore;
 ...
 */
export function findTopMatches(extractedText, limit = 10) {
    const state = store.getState();
    const bonuses = state.configConditionalBonuses.bonuses;
    if (!extractedText || !bonuses)
        return [];
