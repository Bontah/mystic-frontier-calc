 ...
 */
export function calculateRerollSuggestions(
  currentDice: number[],
  familiars: Array<FamiliarContext & { rank: string }>,
  bonusItems: BonusItem[],
  conditionalBonuses: ConditionalBonus[],
  difficulty: number
): RerollSuggestion[] {
  ...
  for (let dieIndex = 0; dieIndex < currentDice.length; dieIndex++) {
    ...
    const passingValues: PassingValue[] = [];
 ...
 */
export function getBestRerollOption(
  suggestions: RerollSuggestion[]
): RerollSuggestion | null {
  const rerollable = suggestions.filter(
    (s) => s.passingValues.length > 0 && !s.currentPasses
  );

  if (rerollable.length === 0) return null;
 ...
 */
export function canPassWithSingleReroll(suggestions: RerollSuggestion[]): boolean {
 ...
 */
export function getRerollSummary(suggestions: RerollSuggestion[]): {
  canPass: boolean;
