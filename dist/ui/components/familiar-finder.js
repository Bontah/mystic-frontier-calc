 ...
 */
export async function initFamiliarFinder() {
    try {
...
function setupEventHandlers() {
    ...
    const remainingToggle = document.getElementById('famfinderShowRemaining');
    if (remainingToggle) {
 ...
 */
function lockSelection(type, element, familiarId) {
 ...
 */
function getAlternatives(type, element) {
    return allFamiliars
        .filter(f => {
        if (ignoredIds.has(f.FamiliarId))
            return false;
        if (currentMode === 'types')
            return f.type === type;
        if (currentMode === 'elements')
 ...
 */
function showAlternatives(type, element, currentId) {
  ...
  `).join('');
    ...
    listEl.querySelectorAll('[data-action="select-alternative"]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const item = e.target.closest('.famfinder-alternative-item');
            if (!item)
                return;
...
function findMinimumFamiliars() {
    try {
        ...
        const available = allFamiliars.filter(f => {
            if (ignoredIds.has(f.FamiliarId))
                return false;
            if (showRemainingOnly && doneFamiliarIds.has(f.FamiliarId))
                return false;
            return true;
        });
        // Greedy set cover algorithm with locked selections
        // Only track what we need based on mode
        const uncoveredTypes = currentMode !== 'elements' ? new Set(TYPES) : new Set();
        const uncoveredElements = currentMode !== 'types' ? new Set(ELEMENTS) : new Set();
        ...
        while (uncoveredTypes.size > 0 || uncoveredElements.size > 0) {
            ...
            for (const fam of available) {
                if (usedIds.has(fam.FamiliarId))
                    continue;
                let score = 0;
                if (currentMode !== 'elements' && uncoveredTypes.has(fam.type))
                    score++;
                if (currentMode !== 'types' && uncoveredElements.has(fam.element))
                    score++;
...
function renderIgnoredSection() {
    ...
    if (ignoredIds.size > 0) {
        ...
        listEl.innerHTML = ignoredFams.map(f => `<span class="famfinder-ignored-tag" data-id="${f.FamiliarId}">${f.MobName} ✕</span>`).join('');
        // Add click handlers
 ...
 */
function renderCoverage(uncoveredTypes, uncoveredElements) {
 ...
 * Render results grid
 */
function renderResults(selectedFamiliars) {
    const resultsDiv = document.getElementById('famfinderResults');
    if (!resultsDiv)
        return;
    resultsDiv.innerHTML = selectedFamiliars.map((fam, i) => {
        const covers = [];
        if (currentMode !== 'elements' && fam.newType)
            covers.push(fam.type);
        if (currentMode !== 'types' && fam.newElement)
            covers.push(fam.element);
    ...
    resultsDiv.querySelectorAll('[data-action="show-alternatives"]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            const currentId = parseInt(e.target.getAttribute('data-current') || '0');
