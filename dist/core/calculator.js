 ...
 */
function toFamiliarContext(familiar) {
    return {
        type: familiar.type,
 ...
 */
export function calculateScore(dice, familiars, bonusItems, conditionalBonuses) {
 ...
 */
export function evaluateLineup(familiars, additionalBonuses, dice) {
    const diceSum = dice.reduce((a, b) => a + b, 0);
    ...
    for (let index = 0; index < familiars.length; index++) {
        ...
        if (fam.conditional) {
            const result = evaluateConditionalBonus(fam.conditional, dice, familiarContexts);
            if (result.isActive) {
 ...
 */
export function calculateExpectedScore(familiars, additionalBonuses, maxDice) {
    let totalScore = 0;
    let count = 0;
