 ...
 */
function isBuggedConditional(bonus) {
    return bonus.prePatch === true;
}
/**
 * Create a conditional selector instance
 */
export function createConditionalSelector(config) {
    const state = {
    ...
    function search() {
        const searchInput = getElement(config.searchInputId);
        const resultsContainer = getElement(config.resultsContainerId);
        const variantSection = getElement(config.variantSectionId);
        ...
        const matches = allBonuses.filter((b) => {
            if (!prePatch && selectedRank && b.rarity !== selectedRank)
                return false;
    ...
    function selectTrigger(index) {
        ...
        const resultsContainer = getElement(config.resultsContainerId);
        const variantSection = getElement(config.variantSectionId);
        const triggerName = getElement(config.triggerNameId);
        const variantPills = getElement(config.variantPillsId);
    ...
    function renderVariantPills(container) {
        if (!container)
            return;
    ...
    function updateDisplay() {
        const display = getElement(config.displayId);
        if (!display)
            return;
    ...
    function showRankBasedTriggers(rank) {
        const allBonuses = store.getState().configConditionalBonuses.bonuses || [];
